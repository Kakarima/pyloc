...
def home(request):
    ...
    display_booking_date_start = display_booking_date_end = \
        booking_date_start = booking_date_end = vehicles = agency = category = customer = None
...
def register_contract(request):
    ...
    if (request.user == None or request.user.is_authenticated == False):
        ...
        already_contracted = \
            Contract.objects.all().filter(agence=agency).filter(vehicle=vehicle).filter(date_end__gt=booking_date_start)
        print('already_contracted = ' + str(already_contracted))

        # erreurs
        error = ''
        if already_contracted.count() > 0:
            ctr_error = 'Impossible de réserver à partir des choix transmis. ' \
                        'Le véhicule demandé n\'est plus disponible.'
        else:
